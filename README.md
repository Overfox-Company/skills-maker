...
Editor de una sola vista para combinar ocho secciones de diseño de las 74 marcas de `dataset/` y descargar un `DESIGN.md`. React + Vite, Tailwind CSS y 18 componentes de shadcn/ui sobre Radix.
...
Los ocho selectores son independientes: colores, tipografía, distribución, espaciado, formas, elevación, botones y campos. Las selecciones se guardan únicamente en este navegador. Los formularios, precios e invitaciones de la galería son ejemplos interactivos; no envían datos a servicios externos.
...
Se aceptan `--colors`, `--typography`, `--layout`, `--spacing`, `--shape`, `--elevation`, `--buttons` y `--inputs`, usando los IDs de `pnpm catalog`. El `DESIGN.md` original de la raíz se conserva intacto.
...
Las pruebas cubren todas las combinaciones marca/sección (74 × 8), independencia entre selectores, recuperación de preferencias, coincidencia entre exportación y vista previa, y presencia de fuentes/licencias locales. La carpeta `output/playwright/` contiene evidencia local de las comprobaciones de navegador y está excluida de Git.
